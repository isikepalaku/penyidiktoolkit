...
serve(async (req) => {
  ...
  try {
    ...
    // Ada dua opsi untuk menangani penolakan pengguna:
    // 1. Hanya update status yang menunjukkan user ditolak
    // 2. Hapus pengguna dari sistem (pendekatan yang ditunjukkan di sini)
    
    // Opsi 1: Update status
    // const { data: updatedUser, error: updateError } = await supabase.auth.admin.updateUserById(
    //   userId,
    //   { user_metadata: { registration_status: 'rejected' } }
    // )

    // Opsi 2: Hapus pengguna (lebih umum untuk pendaftaran yang ditolak)
    const { error: deleteError } = await supabase.auth.admin.deleteUser(userId)

    if (deleteError) {
      console.error('Error deleting user:', deleteError)
      return new Response(JSON.stringify({ error: 'Gagal menolak pendaftaran pengguna' }), {
        status: 500,
        headers: {
          'Content-Type': 'application/json',
          'Access-Control-Allow-Origin': '*',
    ...
    // Opsional: Kirim email ke user bahwa akun mereka telah ditolak
    // Implementasi pengiriman email di sini

    return new Response(
      JSON.stringify({ 
        message: 'Pendaftaran pengguna berhasil ditolak' 
      }),
      {
        status: 200,
        headers: {
...
/* To invoke:
  ...
  3. Use curl or a client to make a POST request:
     curl -i --location --request POST 'http://localhost:54321/functions/v1/admin/reject-user' \
