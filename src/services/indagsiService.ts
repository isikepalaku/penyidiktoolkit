...
export const initializeSession = async () => {
  ...
  if (session?.user?.id) {
    ...
    console.log('INDAGSI: Using authenticated user ID:', currentUserId);
  } 
  // Jika tidak ada session Supabase, gunakan UUID
  else if (!currentUserId) {
    currentUserId = `anon_${uuidv4()}`;
    console.log('INDAGSI: Created new anonymous user ID:', currentUserId);
  }
  
  // Buat session ID baru untuk percakapan jika belum ada
  if (!currentSessionId) {
    currentSessionId = `session_${uuidv4()}`;
    console.log('INDAGSI: Created new session ID:', currentSessionId);
    console.log('INDAGSI: User ID:', currentUserId, 'Session ID:', currentSessionId);
  } else {
    console.log('INDAGSI: Using existing session ID:', currentSessionId);
    console.log('INDAGSI: User ID:', currentUserId, 'Session ID:', currentSessionId);
  }
};
...
export const clearChatHistory = () => {
  console.log('INDAGSI: Clearing chat history and session');
  currentSessionId = null;
  // Tetap mempertahankan user ID untuk konsistensi
  console.log('INDAGSI: Kept user ID:', currentUserId);
};
