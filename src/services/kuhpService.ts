/**
 * KUHP AI Service (Ahli Hukum Pidana)
 * Service untuk menangani chat AI untuk bidang hukum pidana
 * Menggunakan backend API untuk manajemen session dengan dukungan upload file
 */
...
export const initializeSession = async () => {
  ...
  if (session?.user?.id) {
    ...
    console.log('KUHP: Using authenticated user ID:', currentUserId);
  } 
  // Jika tidak ada session Supabase, gunakan UUID
  else if (!currentUserId) {
    currentUserId = `anon_${uuidv4()}`;
    console.log('KUHP: Created new anonymous user ID:', currentUserId);
  }
  
  // Buat session ID baru untuk percakapan jika belum ada
  if (!currentSessionId) {
    currentSessionId = `session_${uuidv4()}`;
    console.log('KUHP: Created new session ID:', currentSessionId);
    console.log('KUHP: User ID:', currentUserId, 'Session ID:', currentSessionId);
  } else {
    console.log('KUHP: Using existing session ID:', currentSessionId);
    console.log('KUHP: User ID:', currentUserId, 'Session ID:', currentSessionId);
  }
};
...
export const clearChatHistory = () => {
  console.log('KUHP: Clearing chat history and session');
  currentSessionId = null;
  // Tetap mempertahankan user ID untuk konsistensi
  console.log('KUHP: Kept user ID:', currentUserId);
};
    ...
    while (retries <= MAX_RETRIES) {
      try {
        ...
        const endpoint = files && files.length > 0 
          ? 'runs-with-files' 
          : 'runs';
        const url = `${API_BASE_URL}/v1/playground/agents/ahli-hukum-pidana/${endpoint}`;
