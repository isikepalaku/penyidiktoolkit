/**
 * WASSIDIK PENYIDIK AI Service
 * Service untuk menangani chat AI untuk bidang pengawasan penyidikan
...
export const initializeSession = async () => {
  ...
  if (session?.user?.id) {
    ...
    console.log('WASSIDIK PENYIDIK: Using authenticated user ID:', currentUserId);
  } 
  // Jika tidak ada session Supabase, gunakan UUID
  else if (!currentUserId) {
    currentUserId = `anon_${uuidv4()}`;
    console.log('WASSIDIK PENYIDIK: Created new anonymous user ID:', currentUserId);
  }
  
  // Buat session ID baru untuk percakapan jika belum ada
  if (!currentSessionId) {
    currentSessionId = `session_${uuidv4()}`;
    console.log('WASSIDIK PENYIDIK: Created new session ID:', currentSessionId);
    console.log('WASSIDIK PENYIDIK: User ID:', currentUserId, 'Session ID:', currentSessionId);
  } else {
    console.log('WASSIDIK PENYIDIK: Using existing session ID:', currentSessionId);
    console.log('WASSIDIK PENYIDIK: User ID:', currentUserId, 'Session ID:', currentSessionId);
  }
};
...
export const clearChatHistory = () => {
  console.log('WASSIDIK PENYIDIK: Clearing chat history and session');
  currentSessionId = null;
  // Tetap mempertahankan user ID untuk konsistensi
  console.log('WASSIDIK PENYIDIK: Kept user ID:', currentUserId);
};
  ...
  try {
    ...
    while (retries <= MAX_RETRIES) {
      try {
        ...
        formData.append('agent_id', 'wassidik-chat');
        ...
        const endpoint = files && files.length > 0 
          ? 'runs-with-files' 
          : 'runs';
        const url = `${API_BASE_URL}/v1/playground/agents/wassidik-chat/${endpoint}`;
