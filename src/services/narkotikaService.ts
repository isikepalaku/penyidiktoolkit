/**
 * NARKOTIKA AI Service
 * Service untuk menangani chat AI untuk bidang tindak pidana narkotika
...
export const initializeSession = async () => {
  ...
  if (session?.user?.id) {
    ...
    console.log('NARKOTIKA: Using authenticated user ID:', currentUserId);
  } 
  // Jika tidak ada session Supabase, gunakan UUID
  else if (!currentUserId) {
    currentUserId = `anon_${uuidv4()}`;
    console.log('NARKOTIKA: Created new anonymous user ID:', currentUserId);
  }
  
  // Buat session ID baru untuk percakapan jika belum ada
  if (!currentSessionId) {
    currentSessionId = `session_${uuidv4()}`;
    console.log('NARKOTIKA: Created new session ID:', currentSessionId);
    console.log('NARKOTIKA: User ID:', currentUserId, 'Session ID:', currentSessionId);
  } else {
    console.log('NARKOTIKA: Using existing session ID:', currentSessionId);
    console.log('NARKOTIKA: User ID:', currentUserId, 'Session ID:', currentSessionId);
  }
};
...
export const clearChatHistory = () => {
  console.log('NARKOTIKA: Clearing chat history and session');
  currentSessionId = null;
  // Tetap mempertahankan user ID untuk konsistensi
  console.log('NARKOTIKA: Kept user ID:', currentUserId);
};
  ...
  try {
    ...
    while (retries <= MAX_RETRIES) {
      try {
        ...
        formData.append('agent_id', 'narkotika-chat');
        ...
        console.log('Sending request with FormData:', {
          ...
          is_authenticated: !currentUserId.startsWith('anon_')
        });
        ...
        const url = `${API_BASE_URL}/v1/playground/agents/narkotika-chat/runs`;
