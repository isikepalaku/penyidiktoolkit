/**
 * Ahli Hukum Pidana Service
 * Service untuk menangani chat AI untuk ahli hukum pidana
...
export const initializeSession = async () => {
  ...
  if (session?.user?.id) {
    ...
    console.log('AHLI_HUKUM_PIDANA: Using authenticated user ID:', currentUserId);
  } 
  // Jika tidak ada session Supabase, gunakan UUID
  else if (!currentUserId) {
    currentUserId = `anon_${uuidv4()}`;
    console.log('AHLI_HUKUM_PIDANA: Created new anonymous user ID:', currentUserId);
  }
  
  // Buat session ID baru untuk percakapan jika belum ada
  if (!currentSessionId) {
    currentSessionId = `session_${uuidv4()}`;
    console.log('AHLI_HUKUM_PIDANA: Created new session ID:', currentSessionId);
    console.log('AHLI_HUKUM_PIDANA: User ID:', currentUserId, 'Session ID:', currentSessionId);
  } else {
    console.log('AHLI_HUKUM_PIDANA: Using existing session ID:', currentSessionId);
    console.log('AHLI_HUKUM_PIDANA: User ID:', currentUserId, 'Session ID:', currentSessionId);
  }
};
...
export const clearChatHistory = () => {
  console.log('AHLI_HUKUM_PIDANA: Clearing chat history and session');
  currentSessionId = null;
  // Tetap mempertahankan user ID untuk konsistensi
  console.log('AHLI_HUKUM_PIDANA: Kept user ID:', currentUserId);
};
  ...
  try {
    ...
    while (retries <= MAX_RETRIES) {
      try {
        ...
        formData.append('agent_id', 'ahli-hukum-pidana');
        ...
        // Use teams endpoint for ahli hukum pidana
        const url = `${API_BASE_URL}/v1/playground/teams/ahli-hukum-pidana-team/runs`;
