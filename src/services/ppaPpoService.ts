/**
 * PPA PPO AI Service
 * Service untuk menangani chat AI untuk bidang Tindak Pidana Perempuan dan Anak dan Pidana Perdagangan Orang
...
export const initializeSession = async () => {
  ...
  if (session?.user?.id) {
    ...
    console.log('PPA-PPO: Using authenticated user ID:', currentUserId);
  } 
  // Jika tidak ada session Supabase, gunakan UUID
  else if (!currentUserId) {
    currentUserId = `anon_${uuidv4()}`;
    console.log('PPA-PPO: Created new anonymous user ID:', currentUserId);
  }
  
  // Buat session ID baru untuk percakapan jika belum ada
  if (!currentSessionId) {
    currentSessionId = `session_${uuidv4()}`;
    console.log('PPA-PPO: Created new session ID:', currentSessionId);
    console.log('PPA-PPO: User ID:', currentUserId, 'Session ID:', currentSessionId);
  } else {
    console.log('PPA-PPO: Using existing session ID:', currentSessionId);
    console.log('PPA-PPO: User ID:', currentUserId, 'Session ID:', currentSessionId);
  }
};
...
export const clearChatHistory = () => {
  console.log('PPA-PPO: Clearing chat history and session');
  currentSessionId = null;
  // Tetap mempertahankan user ID untuk konsistensi
  console.log('PPA-PPO: Kept user ID:', currentUserId);
};
  ...
  try {
    ...
    while (retries <= MAX_RETRIES) {
      try {
        ...
        formData.append('agent_id', 'ppa-ppo-chat');
        ...
        const url = `${API_BASE_URL}/v1/playground/agents/ppa-ppo-chat/runs`;
