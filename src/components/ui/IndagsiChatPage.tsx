...
import { ArrowLeft, Send, Copy, Check, Loader2, ShoppingBag, Info, X } from 'lucide-react';
import { cn } from '@/utils/utils';
import { Button } from './button';
import { Textarea } from './textarea';
import { AnimatedBotIcon } from './animated-bot-icon';
import { DotBackground } from './DotBackground';
import { sendChatMessage, initializeSession, clearChatHistory } from '@/services/penyidikService';
...
const IndagsiChatPage: React.FC<IndagsiChatPageProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    setMessages([
      {
        content: 'Indagsi Ai adalah asisten ai untuk membantu menjawab pertanyaan seputar tindak pidana di bidang Industri dan Perdagangan',
        type: 'bot',
        timestamp: new Date(),
      },
    ]);
  ...
  const exampleQuestions = [
    "Apa yang dimaksud dengan tindak pidana pemalsuan merek?",
    "Bagaimana proses penyelidikan kasus penipuan perdagangan?",
    "Apa saja jenis-jenis tindak pidana di bidang industri?",
    "Bagaimana cara mendeteksi produk palsu di pasaran?"
  ];
        ...
        <header className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="text-lg font-semibold text-gray-900">Indagsi AI</h2>
                <p className="text-xs text-gray-500">Asisten tindak pidana industri dan perdagangan</p>
              </div>
            </div>
          </div>
          <button 
        ...
        {showInfo && (
          <div className="bg-purple-50 border-b border-purple-200 p-3 text-sm text-purple-800">
            <div className="max-w-3xl mx-auto">
              <p className="mb-2"><strong>Tentang Indagsi AI:</strong></p>
              <p>Indagsi AI adalah asisten yang dirancang untuk membantu Anda dengan pertanyaan seputar tindak pidana di bidang Industri dan Perdagangan.</p>
              <p className="mt-2 text-xs text-purple-600">Catatan: Indagsi AI dapat memberikan informasi yang tidak akurat. Verifikasi fakta penting.</p>
            </div>
          </div>
        )}
                ...
                >
                  {message.isAnimating ? (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-500">Indagsi AI sedang mengetik...</span>
                    </div>
                  ) : message.type === 'bot' ? (
        ...
        <div className="border-t border-gray-200 bg-white p-4 md:px-8 pb-6 md:pb-4">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-xs text-gray-500 mt-2 text-center">
              Indagsi AI dapat memberikan informasi yang tidak akurat. Verifikasi fakta penting.
            </p>
          </div>
        </div>
      </div>
