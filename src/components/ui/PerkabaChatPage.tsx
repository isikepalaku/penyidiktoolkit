...
import { sendChatMessage, clearChatHistory, initializeSession } from '@/services/perkabaService';
...
interface PerkabaChatPageProps {
  onBack?: () => void;
}

const PerkabaChatPage: React.FC<PerkabaChatPageProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    setMessages([
      {
        content: 'Selamat datang di SOP Lidik Sidik AI. Saya adalah asisten AI yang fokus pada informasi seputar SOP Lidik Sidik berdasarkan Perkaba Polri. Apa yang ingin Anda tanyakan?',
        type: 'bot',
        timestamp: new Date()
      }
    ]);
  ...
  const formatMessage = (content: string) => {
    ...
    formattedContent = formattedContent.replace(
      urlRegex, 
      '<a href="$1" target="_blank" rel="noopener noreferrer" class="text-purple-600 hover:underline">$1</a>'
    );
  ...
  const exampleQuestions = [
    "Apa yang dimaksud dengan penyelidikan dalam konteks Perkaba Polri?",
    "Bagaimana prosedur penyidikan tindak pidana menurut SOP?",
    "Jelaskan tahapan penyelidikan menurut Perkaba Polri",
  ...
  return (
    <div className="flex h-screen bg-gray-50 lg:pl-64">
      ...
      <div className="flex flex-col flex-1 h-full overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center justify-center w-9 h-9 bg-blue-100 rounded-full">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Asisten SOP Lidik Sidik Perkaba Polri</p>
              </div>
            </div>
          </div>
          <button 
        ...
        {/* Info Panel */}
        {showInfo && (
          <div className="bg-purple-50 border-b border-purple-200 p-3 text-sm text-purple-800">
            <div className="max-w-3xl mx-auto">
              <p className="mb-2"><strong>Tentang SOP Lidik Sidik AI:</strong></p>
              <p>SOP Lidik Sidik AI adalah asisten yang memberikan informasi tentang Standar Operasional Prosedur (SOP) Penyelidikan dan Penyidikan berdasarkan Perkaba Polri.</p>
              <p className="mt-2 text-xs text-purple-600">Catatan: SOP Lidik Sidik AI dapat memberikan informasi yang tidak akurat. Verifikasi fakta penting dengan dokumen resmi.</p>
            </div>
          </div>
        )}
                ...
                >
                  {message.isAnimating ? (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-500">SOP Lidik Sidik AI sedang mengetik...</span>
                    </div>
                  ) : message.type === 'bot' ? (
                    <div className="space-y-0">
                      <div 
                        className="prose prose-sm max-w-none prose-headings:font-semibold prose-headings:text-gray-900 prose-p:text-gray-700 prose-pre:bg-gray-100 prose-pre:text-gray-800 prose-code:text-gray-800 prose-code:bg-gray-100 prose-code:rounded prose-code:px-1 prose-code:py-0.5 prose-code:text-sm prose-code:font-mono prose-a:text-purple-600 prose-a:no-underline hover:prose-a:underline prose-li:text-gray-700 prose-li:marker:text-gray-500 prose-strong:text-gray-900 prose-em:text-gray-700 prose-p:my-0.5 prose-headings:my-0.5 prose-headings:mb-0 prose-ul:my-0.5 prose-ol:my-0.5 prose-li:my-0.5 prose-pre:my-1 leading-tight [&_p]:!my-0.5 [&_br]:leading-none [&_h1+p]:!mt-0.5 [&_h2+p]:!mt-0.5 [&_h3+p]:!mt-0.5"
          ...
          <div className="max-w-3xl mx-auto">
            <div className="relative">
              <Textarea
                ...
                className="resize-none pr-12 py-3 min-h-[56px] max-h-[200px] rounded-xl border-gray-300 focus:border-purple-500 focus:ring-purple-500 shadow-sm"
            ...
            <p className="text-xs text-gray-500 mt-2 text-center">
              SOP Lidik Sidik AI dapat memberikan informasi yang tidak akurat. Verifikasi fakta penting dengan dokumen resmi.
            </p>
          </div>
        </div>
      </div>
...
export default PerkabaChatPage; 