...
interface PerbankanChatPageProps {
  onBack?: () => void;
}

const PerbankanChatPage: React.FC<PerbankanChatPageProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    setMessages([
      {
        content: 'Selamat datang di Asisten UU Perbankan. Saya dapat membantu Anda dengan pertanyaan seputar Undang-Undang Perbankan di Indonesia. Apa yang ingin Anda ketahui?',
        type: 'bot',
        timestamp: new Date(),
      },
    ]);
    ...
    const savedDraft = localStorage.getItem('perbankan_draft_message');
    if (savedDraft) {
  ...
  useEffect(() => {
    if (inputMessage) {
      localStorage.setItem('perbankan_draft_message', inputMessage);
    } else {
      localStorage.removeItem('perbankan_draft_message');
    }
  }, [inputMessage]);
    ...
    <div className="fixed inset-0 z-20 bg-white lg:pl-64 flex flex-col">
      ...
      <div className="flex items-center justify-between p-4 border-b shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-lg font-semibold text-gray-900">UU Perbankan</h1>
            <p className="text-xs text-gray-500 hidden sm:block">Asisten untuk pertanyaan seputar UU Perbankan</p>
          </div>
        </div>
        <button
      ...
      {showInfo && (
        <div className="p-4 bg-indigo-50 border-b border-indigo-100">
          <div className="max-w-3xl mx-auto">
            <h2 className="font-medium text-indigo-800 mb-2">Tentang UU Perbankan</h2>
            <p className="text-sm text-indigo-700">
              Asisten ini dapat membantu Anda dengan pertanyaan seputar Undang-Undang Perbankan di Indonesia, 
              termasuk UU No. 7 Tahun 1992 jo. UU No. 10 Tahun 1998 tentang Perbankan, UU No. 21 Tahun 2008 tentang Perbankan Syariah, 
              dan peraturan terkait lainnya. Anda dapat bertanya tentang jenis-jenis bank, kegiatan usaha bank, 
              pengawasan perbankan, perlindungan nasabah, dan aspek hukum perbankan lainnya.
            </p>
          </div>
        </div>
      )}
                ...
                >
                  ...
                  >
                    {message.isAnimating ? (
                      ...
                      <div>
                        <div
                          className="prose prose-sm max-w-none break-words prose-headings:font-semibold prose-headings:text-gray-900 prose-p:text-gray-700 prose-pre:bg-gray-100 prose-pre:text-gray-800 prose-code:text-gray-800 prose-code:bg-gray-100 prose-code:rounded prose-code:px-1 prose-code:py-0.5 prose-code:text-sm prose-code:font-mono prose-a:text-indigo-600 prose-a:no-underline hover:prose-a:underline prose-li:text-gray-700 prose-li:marker:text-gray-500 prose-strong:text-gray-900 prose-em:text-gray-700 prose-p:my-0.5 prose-headings:my-0.5 prose-headings:mb-0 prose-ul:my-0.5 prose-ol:my-0.5 prose-li:my-0.5 prose-pre:my-1 leading-tight [&_p]:!my-0.5 [&_br]:leading-none [&_h1+p]:!mt-0.5 [&_h2+p]:!mt-0.5 [&_h3+p]:!mt-0.5"
                          ...
                          <div className="mt-2 pt-2 border-t border-gray-200">
                            ...
                            <ul className="text-xs text-gray-500 space-y-1">
                              {message.sourceDocuments.map((doc, idx) => (
                                <li key={idx}>
                                  {doc.metadata.source || 'Dokumen UU Perbankan'}
                                </li>
                              ))}
                            </ul>
                          </div>
            ...
            {messages.length <= 1 && (
              <div className="mt-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {[
                    'Apa perbedaan bank umum dan bank perkreditan rakyat?',
                    'Bagaimana pengaturan tentang rahasia bank?',
                    'Jelaskan tentang prinsip kehati-hatian dalam perbankan',
                    'Apa sanksi bagi pelanggaran UU Perbankan?'
                  ].map((question, idx) => (
                    <button
                      key={idx}
                      className="text-sm bg-white border border-indigo-200 rounded-lg p-3 text-left hover:bg-indigo-50 transition-colors shadow-sm"
                      onClick={() => {
      ...
      <div className="border-t border-gray-200 bg-white p-4 md:px-8 pb-safe">
        <div className="max-w-3xl mx-auto">
          <div className="relative flex w-full cursor-text flex-col rounded-xl border border-gray-300 px-4 py-3 duration-150 ease-in-out shadow-sm focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500 bg-white">
            <textarea
              value={inputMessage}
...
export default PerbankanChatPage; 