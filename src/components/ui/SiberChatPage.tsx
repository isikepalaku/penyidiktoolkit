  ...
  const formatMessage = (content: string) => {
    try {
      ...
      sanitizedHtml = sanitizedHtml.replace(
        /<a /g, 
        '<a target="_blank" rel="noopener noreferrer" class="break-all text-blue-600 hover:text-blue-700" '
      );
                ...
                >
                  ...
                  >
                    {message.type === "bot" && !message.isAnimating ? (
                      <>
                        <div 
                          className="prose prose-sm max-w-none prose-a:break-all prose-a:text-blue-600 prose-img:max-w-full [&_a]:inline-block [&_a]:max-w-[100%] [&_a]:overflow-hidden [&_a]:text-ellipsis"
        ...
        <div className="max-w-3xl mx-auto">
          <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="relative">
            <Textarea
              ...
              className="pl-4 pr-12 py-3 min-h-[56px] max-h-[200px] rounded-xl border border-gray-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 resize-none"
              disabled={isProcessing}
            />
            <Button 
              type="submit"
              size="icon"
              className="absolute bottom-2.5 right-2.5 h-8 w-8 bg-blue-600 hover:bg-blue-700 text-white rounded-lg"
