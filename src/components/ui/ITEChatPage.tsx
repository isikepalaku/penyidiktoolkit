...
import { sendChatMessage, clearChatHistory, initializeSession } from '@/services/iteService';
...
interface ITEChatPageProps {
  onBack?: () => void;
}

const ITEChatPage: React.FC<ITEChatPageProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    setMessages([
      {
        content: 'Selamat datang di ITE AI. Saya adalah asisten AI yang dapat membantu Anda dengan pertanyaan seputar Undang-Undang Informasi dan Transaksi Elektronik (UU ITE) dan regulasi terkait teknologi informasi.',
        type: 'bot',
        timestamp: new Date()
      }
    ]);
  ...
  const exampleQuestions = [
    "Apa yang dimaksud dengan kejahatan siber dalam UU ITE?",
    "Bagaimana aturan tentang transaksi elektronik dalam UU ITE?",
    "Jelaskan tentang sanksi pidana dalam UU ITE untuk penyebaran konten ilegal",
    "Apa saja perubahan penting dalam UU ITE setelah revisi tahun 2016?"
  ];
  ...
  return (
    <div className="fixed inset-0 z-20 bg-white lg:pl-64">
      ...
      <div className="flex flex-col h-full overflow-hidden">
        ...
        <header className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center justify-center w-9 h-9 bg-cyan-100 rounded-full">
                <BookOpen className="w-5 h-5 text-cyan-600" />
              </div>
              <div>
                <h2 className="text-lg font-semibold text-gray-900">ITE AI</h2>
                <p className="text-xs text-gray-500">Asisten UU Informasi dan Transaksi Elektronik</p>
              </div>
            </div>
          </div>
          <button 
        ...
        {/* Info Panel */}
        {showInfo && (
          <div className="bg-cyan-50 border-b border-cyan-200 p-3 text-sm text-cyan-800">
            <div className="max-w-3xl mx-auto">
              <p className="mb-2"><strong>Tentang ITE AI:</strong></p>
              <p>ITE AI adalah asisten yang membantu Anda memahami Undang-Undang Informasi dan Transaksi Elektronik (UU ITE) dan regulasi terkait teknologi informasi di Indonesia.</p>
              <p className="mt-2 text-xs text-cyan-600">Catatan: ITE AI dapat memberikan informasi yang tidak akurat. Verifikasi fakta penting.</p>
            </div>
          </div>
        )}
                ...
                >
                  {message.isAnimating ? (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-500">ITE AI sedang mengetik...</span>
                    </div>
                  ) : message.type === 'bot' ? (
                    <div className="space-y-0">
                      <div 
                        className="prose prose-sm max-w-none prose-headings:font-semibold prose-headings:text-gray-900 prose-p:text-gray-700 prose-pre:bg-gray-100 prose-pre:text-gray-800 prose-code:text-gray-800 prose-code:bg-gray-100 prose-code:rounded prose-code:px-1 prose-code:py-0.5 prose-code:text-sm prose-code:font-mono prose-a:text-cyan-600 prose-a:no-underline hover:prose-a:underline prose-li:text-gray-700 prose-li:marker:text-gray-500 prose-strong:text-gray-900 prose-em:text-gray-700 prose-p:my-0.5 prose-headings:my-0.5 prose-headings:mb-0 prose-ul:my-0.5 prose-ol:my-0.5 prose-li:my-0.5 prose-pre:my-1 leading-tight [&_p]:!my-0.5 [&_br]:leading-none [&_h1+p]:!mt-0.5 [&_h2+p]:!mt-0.5 [&_h3+p]:!mt-0.5"
        ...
        <div className="border-t border-gray-200 bg-white p-4">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-xs text-gray-500 mt-2 text-center">
              ITE AI dapat memberikan informasi yang tidak akurat. Verifikasi fakta penting.
            </p>
          </div>
        </div>
      </div>
