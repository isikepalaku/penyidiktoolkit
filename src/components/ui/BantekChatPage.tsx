...
import { ArrowLeft, Send, Copy, Check, Loader2, Info, X, FileText } from 'lucide-react';
...
const BantekChatPage: React.FC<BantekChatPageProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    setMessages([
      {
        content: 'Selamat datang di Bantek AI. Saya adalah asisten AI yang fokus pada SOP Bantuan Teknis berdasarkan Perkaba Polri. Apa yang ingin Anda tanyakan?',
        type: 'bot',
        timestamp: new Date()
      }
    ]);
                ...
                >
                  {message.isAnimating ? (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-500">Bantek AI sedang mengetik...</span>
                    </div>
                  ) : message.type === 'bot' ? (
